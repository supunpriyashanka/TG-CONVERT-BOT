...
async def start(c, m):
    ...
    await c.send_message(chat_id=m.chat.id,
                         ...
                         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📌Support Group", url=my_father), InlineKeyboardButton("📌Support channel", url=support)]]))
...
async def login(c, m):
    if Config.BOT_PWD:
        ...
        if (len(m.command) < 2) & (m.from_user.id not in Config.LOGGED_USER) & (m.from_user.id not in Config.AUTH_USERS):
            await c.send_message(chat_id=m.chat.id,
                                 text="Use this command for login to this bot. Send the password in the format 👉`/login Bot password`.",
                                 disable_web_page_preview=True,
                                 reply_to_message_id=m.message_id,
                                 parse_mode="markdown")
